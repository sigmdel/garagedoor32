...
void defaultIdx(void) {
  ...
  config.dmtzContactIdx = DMTZ_CONTACT_IDX;
  config.dmtzAutoCloseIdx = DMTZ_AUTOCLOSE_IDX;
  config.dmtzIsColdIdx = DMTZ_ISCOLD_IDX;
  config.dmtzIsDarkIdx = DMTZ_ISDARK_IDX;
}
...
void defaultTimes(void) {
  ...
  config.relayOnTime = RELAY_ON_TIME;
  config.travelTime = TRAVEL_TIME;
  config.closeDelayShort = CLOSE_DELAY_SHORT;
  config.closeDelayLong = CLOSE_DELAY_LONG;
}
...
void defaultThresholds(void) {
  config.darkThreshold = DARK_THRESHOLD;
  config.coldThreshold = COLD_THRESHOLD;
}
