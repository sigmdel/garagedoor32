...
extern void espRestart(int level);
...
void wifiLoop(void) {
  if (wifiConnected != WiFi.isConnected()) {
    ...
    if (wifiConnected) {
      addToLogPf(LOG_INFO, TAG_WIFI, PSTR("WiFi connected to %s as %s (%s) after %d seconds disconnected."),
        WiFi.SSID().c_str(), WiFi.localIP().toString().c_str(), WiFi.getHostname(), seconds);
      if (accessPointUp) {
        stopAp();
        accessPointUp = false;
...
void wifiLogStatus(void) {
  if (WiFi.isConnected())
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Connected to WiFi network %s as %s (%s)"),
...
void wifiConnect() {
  WiFi.setHostname(config.hostname); // must precede WiFi.mode
  WiFi.mode(WIFI_STA);
...
boolean doOtaUpdate(const char *url) {
  addToLogPf(LOG_INFO, TAG_WIFI, PSTR("Attempting OTA update from %s"), url);
  ...
  int res = httpUpdate.update(OtaClient, url);
  //delay(5);
  ...
  switch(res) {
    case HTTP_UPDATE_FAILED:
      addToLogPf(LOG_ERR, TAG_WIFI, PSTR("OTA update failed. Error (%d): %s"), httpUpdate.getLastError(), httpUpdate.getLastErrorString().c_str());
      break;

    case HTTP_UPDATE_NO_UPDATES:
      addToLogP(LOG_ERR, TAG_WIFI, PSTR("OTA update failed. Error: No updates"));
      break;

    case HTTP_UPDATE_OK:
      addToLogP(LOG_INFO, TAG_WIFI, PSTR("OTA update successful. Restarting..."));
      updated = true;
      break;
  }
  return updated;
