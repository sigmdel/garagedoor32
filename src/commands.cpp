...
// BUG - restart 0 from console results in boot loop
...
static const char* cmds[] = {
  ...
  "thresh",         // - cold & dark threshold values
  "time",           // - configure time intervals
  "topic",          // - mqtt topics
  "wifi",           // - returns wifi status
  ""                // - extra (force door relay perhaps)
};
...
static const char *params[COMMAND_COUNT] = {
  ...
  /* door    */ "close",
  /* help    */ "[<command>]",
  /* idx     */ "[-d] | [(switch|temp|lux|contact|auto|cold|dark) [<id>]]",
  ...
  /* thresh  */ "[-d] | [(cold|dark) [<value>]]",
  /* time    */ "[-d] | [(poll|update|http|ap|short|long) [<ms>]]",
...
// auto  (on | off) xtra
//
cmndError_t doAuto(int count, int &errIndex) {
  errIndex = 1;
  if (count == 2) {
    token[1].toLowerCase();
...
///  1        2    3       2      3       2     2      3     4       <<< count
///  0        1    2       1      2       1     1      2     3       <<< errIndex
...
cmndError_t doDoor(int count, int &errIndex) {
  errIndex = 1;
  if (count < 2)
    return etMissingParam;
  token[1].toLowerCase();
  if (token[1].equals("close"))
    closeDoor();
  else
    return etInvalidValue;
  if (count > 2) {
...
void showIdx(void) {
  addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Domoticz virtual Idx: switch = %d, light sensor = %d, temp + humid sensor = %d, contact = %d, autoclose = %d, cold = %d, dark = %d"),
    config.dmtzSwitchIdx, config.dmtzLSIdx, config.dmtzTHSIdx, config.dmtzContactIdx, config.dmtzAutoCloseIdx, config.dmtzIsColdIdx, config.dmtzIsDarkIdx);
}

//   1   2              2       3       4  <<< count
//   0   1              1       2       3  <<< index
// idx [-d] xtra1 | [(option) [<id>]] xtra2
// option = switch|temp|lux|contact|auto|dark|cold
//
cmndError_t doIdx(int count, int &errIndex) {
  ...
  } else if (token[1].equals("contact")) {
    if (setvalue) config.dmtzContactIdx = anIdx;
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("contact switch idx: %d"), config.dmtzContactIdx);

  } else if (token[1].equals("auto")) {
    if (setvalue) config.dmtzAutoCloseIdx = anIdx;
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("autoclose selector idx: %d"), config.dmtzAutoCloseIdx);

  } else if (token[1].equals("dark")) {
    if (setvalue) config.dmtzIsDarkIdx = anIdx;
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("dark setpoint idx: %d"), config.dmtzIsDarkIdx);

  } else if (token[1].equals("cold")) {
    if (setvalue) config.dmtzIsColdIdx = anIdx;
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("cold setpoint idx: %d"), config.dmtzIsColdIdx);
...
void showThresh(void) {
  addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Cold threshold = %d °C, dark threshold = %d lux"), config.coldThreshold, config.darkThreshold);
}


//   1       2     3        2        3         4      <<< count
//   0       1     2        1        2         3      <<< index
//  thresh [-d] xtra1 | [[option] [<value]]  xtra2
//  where option = cold|dark
//
cmndError_t doThresh(int count, int &errIndex) {
  ...
  if (token[1].equals("cold")) {
    if (value) {
      config.coldThreshold = value;
      updateDomoticzSetPoint(config.dmtzIsColdIdx, value);
    }
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Cold threshold: %d °C"), config.coldThreshold);

  } else if (token[1].equals("dark")) {
    if (value) {
      config.darkThreshold = value;
      updateDomoticzSetPoint(config.dmtzIsDarkIdx, value);
    }
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Dark threshold: %d lux"), config.darkThreshold);
...
//   1    2                   2                    3       4  <<< count
//   0    1                   1                    2       3 <<< errIndex
// time [-d] | [(poll|update|http|ap|short|long) [<ms>]]  xtra
//
cmndError_t doTime(int count, int &errIndex) {
  ...
  } else if (token[1].equals("short")) {
    if (count > 2) { config.closeDelayShort = aTime; }
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Short delay before auto closing door: %d ms"), config.closeDelayShort);

  } else if (token[1].equals("long")) {
    if (count > 2) { config.closeDelayLong = aTime; }
    addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("Long delay before auto closing door: %d ms"), config.closeDelayLong);
...
void showTopics(void) {
  ...
  addToLogPf(LOG_INFO, TAG_COMMAND, PSTR("\x35h\x35 = %s"), config.hostname);
}
...
cmndError_t doXtra(int count, int &errIndex) {
  errIndex = 2; // assume xtra1
  if (count > 2)
    return etExtraParam;
  else
    addToLogP(LOG_INFO, TAG_COMMAND, PSTR("Nothing defined"));
  return etNone;
}
