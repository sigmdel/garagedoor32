...
void Version_t::fromString(String value) {
  int n = sscanf(value.c_str(), "%u.%u.%u", &major, &minor, &patch);
  if (n < 3) patch = 0;
  if (n < 2) minor = 0;
  if (n < 1) major = 0;
}
...
String FirmwareVersion(void) {
  ...
  str.replace("  ", " ");
    ...
    str += " for ARDUINO_ESP32_DEV (az-delivery-devkit-v4)";
  #elif defined(ARDUINO_LOLIN32_LITE)
    str += " for ARDUINO_LOLIN32_LITE";
  #elif defined(ARDUINO_XIAO_ESP32C3)
    str += " for ARDUINO_XIAO_ESP32C3";
  #endif
  return str;
}
